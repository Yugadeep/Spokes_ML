...
def train_model(X, Y, SIZE_X, SIZE_Y, epoch_num, model_path, batch_size=5):
    ...
    model = sm.Unet(backbone_name='resnet34',
                    encoder_weights=None, input_shape=(SIZE_Y, SIZE_X, N))
...
def test_model(model_path, testim_path, SIZE_X, SIZE_Y, epoch_size, training_size):
    model = keras.models.load_model(
        f"{model_path}{training_size}im_{epoch_size}b.h5", compile=False)
    ...
    plt.imsave(
        f'../training/validation/val_{filename}.png', prediction_image, cmap='gray')
